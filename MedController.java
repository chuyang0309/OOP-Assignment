...
public class MedController {
    ...
    @FXML private TextField medName; //get nodes from fxml file according to id
    @FXML private TextField manufacturer; 
    @FXML private TextField expiryDate; 
    @FXML private TextField medCost; 
    @FXML private TextField medUnits; 
    @FXML private TextArea existingMeds; 
    ...
    private void addNewMed(ActionEvent event) {
        ...
        if (validation.isEmpty(medName, "Name")) return;
        if (!validation.isAlphabetic(medName, "Name")) return; 
        if (validation.isEmpty(manufacturer, "Manufacturer")) return;
        if (!validation.isAlphabetic(manufacturer, "Manufacturer")) return; 
//        if (!validation.isEmpty(expiryDate, "Date")) return; 
        if (!validation.isValidDate(expiryDate, "Date")) return; 
        if (validation.isEmpty(medCost, "Cost")) return;
        if (!validation.isNumeric(medCost, "Cost")) return;
        if (validation.isEmpty(medUnits, "Units")) return;
        if (!validation.isNumeric(medUnits, "Units")) return;
        clearFields();
